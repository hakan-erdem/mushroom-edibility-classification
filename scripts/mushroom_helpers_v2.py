...
def trainNetwork(network, optimizer_backbone, optimizer_sequential, loss_fn, train_loader, device):
...
def experiment_pipeline(network, train_set, val_set, test_set, optimizer1, optimizer2,
                        loss_fn, batch_size, epochs, input_size, device, check_overfit=True):
    # loaders
    ...
    for e in range(epochs):
        network.train()
        acc, l = trainNetwork(network, optimizer1, optimizer2, loss_fn, train_loader, device)
        ...
        if check_overfit and train_accs[-1] > 90:
            if overfit(val_accs, val_losses):
                overfit_count += 1

            if overfit_count == 5:
                print(f"Overfit occurred {overfit_count} times, terminating training!")
                break
...
# PRE-TRAINED MODELS
class AlexNetBackBonedV2(nn.Module):
    def __init__(self):
        super(AlexNetBackBonedV2, self).__init__()
        # Load pretrained alexnet
        self.backbone = torch.hub.load("pytorch/vision:v0.10.0", "alexnet", weights="DEFAULT")
...
class VGG19BNBackBonedV2(nn.Module):
    def __init__(self):
        super(VGG19BNBackBonedV2, self).__init__()
        # Load pretrained vgg19_bn
        self.backbone = torch.hub.load("pytorch/vision:v0.10.0", "vgg19_bn", weights="DEFAULT")
...
class DenseNetBackBonedV2(nn.Module):
    def __init__(self):
        super(DenseNetBackBonedV2, self).__init__()
        # Load pretrained densenet
        self.backbone = torch.hub.load("pytorch/vision:v0.10.0", "densenet161", weights="DEFAULT")
...
class ResnetBackBonedV2(nn.Module):
    def __init__(self):
        super(ResnetBackBonedV2, self).__init__()
        # Load pretrained resnet
        self.backbone = torch.hub.load("pytorch/vision:v0.10.0", "resnet50", weights="DEFAULT")
...
class MobileNetV3BackBonedV2(nn.Module):
    def __init__(self):
        super(MobileNetV3BackBonedV2, self).__init__()
        # Load pretrained mobilenet
        self.backbone = torch.hub.load("pytorch/vision:v0.10.0", "mobilenet_v3_large", weights="DEFAULT")## VGG19-BN
